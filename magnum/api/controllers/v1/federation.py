...
class Federation(base.APIBase):
    ...
    # UUID of the master cluster of the federation, i.e. the cluster that
    # hosts the COE Federated API.
    master_id = wsme.wsattr(wtypes.text)
    ...
    def _convert_with_links(federation, url, expand=True):
        if not expand:
            federation.unset_fields_except(['uuid', 'name', 'master_id',
                                            'member_ids', 'status',
                                            'properties'])
    ...
    def sample(cls, expand=True):
        sample = cls(uuid='4221a353-8368-475f-b7de-3429d3f724b3',
                     name='example',
                     master_id='49dc23f5-ffc9-40c3-9d34-7be7f9e34d63',
...
class FederationsController(base.Controller):
    ...
    def post(self, federation):
        ...
        # Validate `master_id`
        master_id = federation_dict.get('master_id')
        attr_validator.validate_federation_master_cluster(master_id)
