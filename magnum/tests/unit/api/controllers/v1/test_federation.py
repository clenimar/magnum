...
class TestPost(api_base.FunctionalTest):
    def setUp(self):
        ...
        self.addCleanup(p.stop)
        self.master_cluster = obj_utils.create_test_cluster(self.context)
    ...
    def test_create_federation(self, mock_utcnow):
        bdict = apiutils.federation_post_data(
            uuid=uuidutils.generate_uuid(), master_id=self.master_cluster.uuid)
    ...
    def test_create_federation_no_master_id(self):
    ...
    def test_create_federation_master_does_not_exist(self):
        bdict = apiutils.federation_post_data(
            uuid=uuidutils.generate_uuid(), master_id=uuidutils.generate_uuid()
        )
        response = self.post_json('/federations', bdict, expect_errors=True)
        self.assertEqual(404, response.status_int)
        self.assertEqual('application/json', response.content_type)
    ...
    def test_create_federation_generate_uuid(self):
        bdict = apiutils.federation_post_data(
            master_id=self.master_cluster.uuid)
        del bdict['uuid']
        response = self.post_json('/federations', bdict)
        self.assertEqual(202, response.status_int)
    ...
    def test_create_federation_with_valid_name(self):
        ...
        for value in valid_names:
            bdict = apiutils.federation_post_data(
                name=value, master_id=self.master_cluster.uuid)
...
class TestDelete(api_base.FunctionalTest):
    ...
    def test_delete_federation(self):
        delete = self.delete('/federations/%s' % self.federation.uuid,
                             expect_errors=True)
